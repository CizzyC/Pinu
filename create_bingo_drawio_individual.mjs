...
const baseDir = "/Users/cizzyc/Documents/Pinu";
...
function writeDrawio(filename, name, cells, width, height) {
  const now = new Date().toISOString();
  const diagram = `<diagram id="${esc(filename)}" name="${esc(name)}">${model(cells, width, height)}</diagram>`;
  const file = `<?xml version="1.0" encoding="UTF-8"?><mxfile host="app.diagrams.net" modified="${now}" agent="Codex" version="24.7.17" type="device">${diagram}</mxfile>`;
  const out = `${baseDir}/${filename}.drawio`;
...

{
  ...
  writeDrawio("Bingo流程图_1_入口与模式分流", "入口与模式分流", c, 1280, 940);
}

{
  ...
  writeDrawio("Bingo流程图_2_单局游戏状态流转", "单局游戏状态流转", c, 1280, 1520);
}

{
  ...
  writeDrawio("Bingo流程图_3_目标分与XP结算", "目标分与 XP 结算", c, 1280, 1540);
}

{
  ...
  writeDrawio("Bingo流程图_4_棋盘选择与词量判断", "棋盘选择与词量判断", c, 1280, 1240);
}
